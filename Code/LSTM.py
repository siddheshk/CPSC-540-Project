...
class lstm(nn.Module):
    ...
    def forward(self, input_indices, sentence_length):
        ...
        sentence_encoding = lstm_hidden[0].transpose(0,1).index_select(0, perm_idx_resort).view(params['batch_size'],1,-1)
        
        linear_input = F.tanh(self.dropout(sentence_encoding))
...
np.random.seed(1)
model = lstm(params, w2v_embeddings)
...
for epoch in range(params['epochs']):
    ...
    for batch in range(number_of_batches):
        ...
        predictions = model(batch_sentences, batch_len)
