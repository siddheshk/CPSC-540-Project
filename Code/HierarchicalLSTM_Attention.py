...
class HierarchicalLSTM(nn.Module):
    def __init__(self, params, embedding_weights=None):
        ...
        if self.params['bidirectional']:
            ...
            self.sentence_matrix = nn.Linear(self.params['hidden_dim']*2, self.params['hidden_dim']*2)
            self.sentence_attention = nn.Linear(self.params['hidden_dim']*2, 1)
        else:
            ...
            self.sentence_matrix = nn.Linear(self.params['hidden_dim'], self.params['hidden_dim'])
            self.sentence_attention = nn.Linear(self.params['hidden_dim'], 1)
    ...
    def forward(self, input_indices, sentence_length, number_of_sentences):
        ...
        sentence_encoding_attended = torch.sum(torch.mul(sentence_encoding, word_att_softmax.expand_as(sentence_encoding)),1).view(batch_size, self.params['number_of_sentences'], -1)
        ...
        sentence_embedding_sorted = sentence_encoding_attended.index_select(0, perm_num_sentence_idx)
        ...
        sentence_lstm_out, _ = nn.utils.rnn.pad_packed_sequence(sentence_lstm_out_packed)
        document_encoding = sentence_lstm_out.transpose(0,1).index_select(0, perm_num_sentence_idx_resort)
        sentence_mask = 1 - document_encoding.eq(0).float()
        sentence_att_mask = 1 - torch.sum(sentence_mask,2).eq(0).float()
        sentence_ui = F.tanh(torch.mul(self.sentence_matrix(document_encoding), sentence_mask))
        sentence_att = torch.mul(torch.exp(self.sentence_attention(sentence_ui)),sentence_att_mask.unsqueeze(2))
        sentence_att_sum = torch.sum(sentence_att,1,keepdim=True).expand_as(sentence_att)
        sentence_att_softmax = torch.div(sentence_att, sentence_att_sum)
        document_encoding_attended = torch.sum(torch.mul(document_encoding, sentence_att_softmax.expand_as(document_encoding)),1)
        #document_encoding = sentence_lstm_hidden[0].transpose(0,1).index_select(0, perm_num_sentence_idx_resort).view(batch_size, -1)

        linear_input = F.tanh(self.dropout(document_encoding_attended))
